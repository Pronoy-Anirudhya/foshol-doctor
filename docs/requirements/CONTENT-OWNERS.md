...
**Hackathon only — does not satisfy C1/C6/C15 English copy.** [`V114__demo_bilingual_catalogue.sql`](../../app/src/main/resources/db/migration/V114__demo_bilingual_catalogue.sql) adds `description_en` and remedy `*_en` columns and fills English translations of **existing** V18/V113 Bangla only. No new dosages, PHI or rates. Those values are **not** content-owner sign-off and must not be quoted as DAE/BRRI/BARI advice.
