...
**Hackathon only — does not satisfy C1/C3/C4/C6/C7/C8.** [`V113__demo_corn_wheat_kb.sql`](../../app/src/main/resources/db/migration/V113__demo_corn_wheat_kb.sql) replaces the V109 `TODO(content-owner)` corn/wheat names, symptoms, phrases and remedies so the officer console has rows after a ViT diagnosis. Those values are **not** content-owner sign-off, **not** production KDB, and must not be quoted as DAE/BRRI/BARI/CIMMYT advice. Replace with human-owned C1–C8 before any real farmer deployment.
