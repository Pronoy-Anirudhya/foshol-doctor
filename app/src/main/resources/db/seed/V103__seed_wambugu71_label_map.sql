-- LOCAL/DEMO seed: extend taxonomy for wambugu71/crop_leaf_diseases_vit label map.
-- Product scope remains rice/tomato/potato; wheat/corn are test scaffolding only.
-- TODO(content-owner: C1/C2/C6) replace English scaffolds and stub remedies before any farmer-facing use.
-- Invalid is intentionally left unmapped.
-- Requires foshol.knowledge.expected-disease-count=20 (see application-local.properties).
...
-- Wheat / corn diseases (6) — brings live disease count from 14 → 20
...
-- Minimal disease_symptom weights (validator requires ≥1 weight row for non-healthy)
...
-- Stub CULTURAL remedies so PRIMARY can fire (not farmer-facing advice)
-- TODO(content-owner: C6)
INSERT INTO remedy (
...
-- Label map for all model classes except Invalid
